...
/* connection a la base de donnée*/
   include("connection_bdd.php");
      ...
      $rep = $bdd->query("SELECT iduser,nom,libel FROM utilisateur,ligue where etat = 1
  ...
  <body>
    <script>
        ...
        function submitForm(rep) {
            $.ajax({type:'POST', url: 'traitmt_list_util.php', data:$('#'+rep).serialize(), success: function(response) {
        ...
        $i = 0;
         // On affiche chaque entrée une à une
          ...
          { 
            ...
            $trouv= 1;
          ?>
            <form method="post" action="traitmt_list_util.php" id="ContactForm<?php echo $i;?>" onsubmit="return submitForm('ContactForm<?php echo $i;?>');">
            ...
            <button class="btn btn-default hide_border" type="submit"  value="supprimer">
            Supprimer</button>
            </div>
          ...
          if (!$trouv) {
            $html = "<h3 style='color:#CCCCCC'>Désolé : aucun utilisateur </h3>"; 
            echo($html);
          }
          ?>
      </div>
